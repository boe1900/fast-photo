...
- 登录失败限流：同一用户名在 5 分钟窗口内连续失败达到阈值后，接口返回 `429`，需稍后重试。
- 登录/注册会写入活动日志（`login` / `register`）。
...
| 端点 | 方法 | 说明 |
|------|------|------|
| `/api/settings/storage` | GET | 获取当前用户存储配置（未配置时返回本地默认） |
| `/api/settings/storage` | PUT | 保存当前用户存储配置 |
| `/api/settings/storage/test` | POST | 测试给定配置连通性 |
...
1. 进入 **设置 → 存储配置**
2. 本地存储路径可编辑，点击「测试本地连接」应显示成功/失败提示
3. 点击「保存本地配置」后应提示保存成功
4. S3 和 WebDAV 卡片应展示配置表单，并可执行「测试连接」「保存配置」

> 注意：当前已实现“按用户保存配置 + 连通性测试”。扫描/上传主流程仍默认使用本地文件系统，远程存储读写切换尚未接入业务主链路。
